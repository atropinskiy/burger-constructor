...
import { IOrder, OrderStatus } from '@customTypes/auth/types';
import {
	FormattedDate,
	CurrencyIcon,
			...
			<div className='w-100 d-flex justify-between'>
				<div className={s.order__ingredients}>
					{visibleIngredients.map((id, index) => {
						const ingredient = ingredients.allItems.find(
							(item) => item._id === id
						);
						if (!ingredient) return null;

						const isLastVisible =
							index === visibleIngredients.length - 1 && hiddenCount > 0;
						...
						return (
							<div
								...
								role={isLastVisible ? 'button' : undefined}
								onKeyDown={(e) => {
									if ((e.key === 'Enter' || e.key === ' ') && isLastVisible) {
										handleShowAll();
									}
								}}
								onClick={isLastVisible ? handleShowAll : undefined}>
								<img
									className={s.order__ingredients__image}
