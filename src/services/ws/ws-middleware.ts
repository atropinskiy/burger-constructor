...
export const createWebSocketMiddleware = ({ url, actionPrefix, requireToken = false }: WSOptions): Middleware => {
  ...
  return store => next => action => {
    ...
    const openType = `${actionPrefix}/open`;
    ...
    switch (action.type) {
      ...
      case sendType: {
        if (socket && socket.readyState === WebSocket.OPEN) {
          const { payload } = action as unknown as { payload: any };
