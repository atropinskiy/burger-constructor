...
describe('orderSlice', () => {
  ...
  it('should handle clearOrder', () => {
    const modifiedState = { ...initialState, ingredients: ['item1'], error: 'some error' };
  ...
  it('should handle removeBunsFromOrderById', () => {
    const modifiedState = { ...initialState, ingredients: ['bun1', 'bun1', 'item'] };
  ...
  it('should handle createOrder.fulfilled', () => {
    ...
    const state = reducer(initialState, createOrder.fulfilled(payload, '', ['ingredient1', 'ingredient2']));
    expect(state.number).toBe(123);
  });

  it('should handle createOrder.rejected', () => {
    const state = reducer(initialState, createOrder.rejected({} as any, '', ['ingredient1', 'ingredient2'], 'Ошибка создания'));
  ...
  it('should handle fetchOrders.fulfilled', () => {
    ...
    const state = reducer(initialState, fetchOrders.fulfilled(payload, '', undefined));
    expect(state.orders).toEqual(mockOrders);
  });

  it('should handle fetchOrders.rejected', () => {
    const state = reducer(initialState, fetchOrders.rejected({} as any, '', undefined, 'Ошибка при загрузке заказов'));
